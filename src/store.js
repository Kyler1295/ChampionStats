...
export default new Vuex.Store({
  state: {
    ...
    version: '',
    // States when the site is loaded
    listLoaded: false
  },
  mutations: {
    // Gets the Champions from the API.
    addChampions (state, {champion}) {
  ...
  actions: {
    getChampions({commit}){
      return new Promise((resolve, reject) => {
        ...
        axios.get('https://ddragon.leagueoflegends.com/cdn/' + this.state.version + '/data/en_US/champion.json')
        ...
        .then(champions => {
          champions.forEach(champion => {
            ...
            axios.get('https://ddragon.leagueoflegends.com/cdn/' + this.state.version + '/data/en_US/champion/' + champion[0] + '.json')
            .then( response => response.data)
            .then( data => Object.entries(data))
            .then( data=> data[3][1])
            .then( data => Object.entries(data))
            .then ( data => {
              ...
              let newChampion = {
                ...
                championImage: 'https://ddragon.leagueoflegends.com/cdn/' + this.state.version + '/img/champion/' + data[0][1].image.full,
              ...
              this.state.champions.push(newChampion);
            })
            
          });
        })
        .then( data => this.state.listLoaded = true)
        .then( data => resolve())
